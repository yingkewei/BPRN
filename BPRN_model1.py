...
class ParallelTripleEncoder(nn.Module):
    ...
    def __init__(self, in_channel=1, first_out_channel=16, use_lightweight=True,
                 sacb_clusters=4, use_sacb_in_encoder=False):
        super().__init__()
        c = first_out_channel
        ...
        self.sacb2_refine = SimplifiedSACB(4 * c, 4 * c, num_clusters=sacb_clusters, residual=True)
        ...
        # SACB for feature refinement (preserves 8*c channels)
        sacb3_block = SimplifiedSACB if not use_lightweight else LightweightSACB
        self.sacb3_refine = sacb3_block(8 * c, 8 * c, num_clusters=sacb_clusters, residual=True)
        ...
        self.use_sacb_in_encoder = use_sacb_in_encoder
...
class SACB_AdaptiveFusion(nn.Module):
    ...
    def __init__(self, in_channels, use_lightweight=True, sacb_clusters=3):
        super().__init__()
        SACB_Block = LightweightSACB if use_lightweight else SimplifiedSACB
        self.reduce = nn.Conv3d(in_channels, in_channels, 1)
        self.sacb = SACB_Block(in_channels, in_channels, num_clusters=sacb_clusters, residual=False)
...
class BPRN(nn.Module):
    def __init__(self, inshape=(160, 192, 160), flow_multiplier=1., in_channel=1, channels=16,
                 ...
                 sacb_clusters=4, sacb_fusion_clusters=3, use_sacb_in_encoder=False):
        super().__init__()
        ...
        self.cconv_4 = nn.Sequential(
            ...
            SACB_Block(8 * c, 8 * c, num_clusters=sacb_clusters, residual=True)
        )
        ...
        self.dconv4 = nn.Sequential(
            SACB_Block(3 * 8 * c, 8 * c, num_clusters=sacb_clusters, residual=False),
            EnhancedConvBlock(8 * c, 8 * c)
        )

        # 第3层解码器 (2次递归 - 第1次用SACB，第2次用标准卷积)
        self.upconv3 = UpConvBlock(8 * c, 4 * c, 4, 2)
        self.adaptive_fusion_3 = SACB_AdaptiveFusion(
            3 * 4 * c,
            use_lightweight=use_lightweight_sacb,
            sacb_clusters=sacb_fusion_clusters
        )
        ...
        self.dconv3 = SACB_Block(3 * 4 * c, 4 * c, num_clusters=sacb_clusters, residual=False)

        # 第2层解码器 (3次递归 - 第一次用SACB，后续用标准卷积)
        self.upconv2 = UpConvBlock(3 * 4 * c, 2 * c, 4, 2)
        # 第一次融合：使用SACB
        self.adaptive_fusion_2_first = SACB_AdaptiveFusion(
            3 * 2 * c,
            use_lightweight=use_lightweight_sacb,
            sacb_clusters=sacb_fusion_clusters
        )
        ...
        # 第一次dconv：使用SACB
        self.dconv2_first = SACB_Block(3 * 2 * c, 2 * c, num_clusters=sacb_clusters, residual=False)
        # 递归dconv：使用标准卷积
...
class BPRN_Lite(nn.Module):
    ...
    def __init__(self, inshape=(160, 192, 160), flow_multiplier=1., in_channel=1, channels=12,
                 sacb_clusters=2, sacb_fusion_clusters=2, use_sacb_in_encoder=False):
        super().__init__()
        self.flow_multiplier = flow_multiplier
        self.channels = channels
        self.step = 7
        ...
        self.encoder_moving = ParallelTripleEncoder(
        ...
        self.encoder_fixed = ParallelTripleEncoder(
            in_channel=in_channel,
            first_out_channel=c,
            use_lightweight=True,
            sacb_clusters=sacb_clusters,
        ...
        self.cconv_4 = nn.Sequential(
            ...
            LightweightSACB(8 * c, 8 * c, num_clusters=sacb_clusters, residual=True)
        )
        ...
        self.defconv4.bias = nn.Parameter(torch.zeros(self.defconv4.bias.shape))
        self.dconv4 = nn.Sequential(
            LightweightSACB(3 * 8 * c, 8 * c, num_clusters=sacb_clusters, residual=False),
            EnhancedConvBlock(8 * c, 8 * c)
        )
        ...
        self.defconv3.weight = nn.Parameter(Normal(0, 1e-5).sample(self.defconv3.weight.shape))
        self.defconv3.bias = nn.Parameter(torch.zeros(self.defconv3.bias.shape))
        self.dconv3 = LightweightSACB(3 * 4 * c, 4 * c, num_clusters=sacb_clusters, residual=False)
        ...
        self.defconv2.weight = nn.Parameter(Normal(0, 1e-5).sample(self.defconv2.weight.shape))
        self.defconv2.bias = nn.Parameter(torch.zeros(self.defconv2.bias.shape))
        self.dconv2_first = LightweightSACB(3 * 2 * c, 2 * c, num_clusters=sacb_clusters, residual=False)
        self.dconv2_recur = nn.Sequential(
            ConvInsBlock(3 * 2 * c, 2 * c, 3, 1),
            ConvInsBlock(2 * c, 2 * c, 3, 1)
        )
